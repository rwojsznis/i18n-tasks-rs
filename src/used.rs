...
pub struct UsedKeys {
    /// Resolved keys and every place they are used, sorted by key.
    pub keys: BTreeMap<String, Vec<Occurrence>>,
    /// Fast membership test for `unused`.
...
impl UsedKeys {
    ...
    pub fn key_used(&self, key: &str) -> bool {
        ...
        while let Some(parent) = crate::keys::parent_key(k) {
            if self.key_set.contains(parent) {
                return true;
            }
            k = parent;
        }
    ...
    pub fn from_scan(scan: FileScan, files_scanned: usize, files_prefiltered: usize) -> UsedKeys {
        ...
        let key_set: HashSet<String> = keys.keys().cloned().collect();
