...
about.team.johnny.bio_html                     used_in_expr? = true    (.slim)
```
...
## Two port bugs found by porting the remaining specs, both fixed

Neither is an accepted difference. They are recorded here because the specs that
catch them were the ones still missing, and the fixes changed behaviour.

### `available_locales` guessed the locale instead of matching the pattern

`spec/file_system_data_spec.rb#available_locales` runs three read patterns over
the same three files and expects three different answers. The port anchored on
the literal tail of the pattern prefix, which cannot tell
`config/locales/%{locale}.yml` from `config/locales/*%{locale}.yml`: the glob
reaches `other.fr.yml` under both, and the heuristic read `fr` out of it under
both. So a project whose only read pattern is `config/locales/%{locale}.yml`
inferred a locale `fr` the gem never sees, built an empty tree for it, and
`missing --types diff` then reported every base key against it.

`file_system_base.rb:122-124` builds an anchored regex instead —
`%{locale}` becomes `([^/.]+)`, which a dotted name cannot satisfy — and
`src/data/load.rs#locale_pattern_re` now does the same, `**` and all. The three
spec cases are `the_read_pattern_decides_which_files_name_a_locale`.

### A file named in `search.paths` was never scanned

`spec/scanners/files/file_finder_spec.rb` passes `paths: %w[a/a a/b/a.txt]` and
expects `a/b/a.txt` among the results, because `Find.find` yields a path handed
to it directly. The port called `read_dir` on every entry in `search.paths` and
silently found nothing for a file. `Finder::consider` now holds the per-file
decision and both the walk and a direct path go through it, so a named file is
subject to the same hidden, `only` and `exclude` rules the gem applies to it.

All four differential reports are unchanged by both fixes.

---

## Implementation choices that are not behaviour differences
...
- **Rails context detection is always on.** The gem gates controller, mailer and
  ...
  dropped anyway, and the flag is not part of the new config format.

---

## The parallel scan

No accepted difference. `rayon` changed no behaviour, which is the point: every
differential report is byte-identical to its single-threaded version, and
`tests/jobs.rs` holds the output identical at every `--jobs` setting.
